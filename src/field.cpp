...
env::MoveResult field::mark(size_t x, size_t y) {
    ...
    if (cell_status == env::CellStatus::CLOSED) {
        ...
        if (_field[cur_position] == env::Cell::BOMBS_0) {
            for (int dx = -1; dx <= 1; dx++) {
                for (int dy = -1; dy <= 1; dy++) {
                    ...
                    if (_field_status[to_position(x + dx, y + dy)] == env::CellStatus::CLOSED) {
                        to_visit.emplace_back(x + dx, y + dy);
                    }
                }
            }
        }
...
std::pair<env::MoveResult, std::vector<coordinate>> field::open(size_t x, size_t y) {
    ...
    if (cell == env::Cell::BOMB) {
        ...
        game_status = env::GameStatus::LOSE;
    ...
    if (closed_cells == 0) {
        game_status = env::GameStatus::WIN;
